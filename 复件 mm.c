...
inline void *find_fit(size_t asize){
	...
	for(int i = free_list_no; i < FREE_LIST_NUM; i++){
		...
		if(first_block == 0)
			continue;
		for(bp = first_block; (unsigned long int)bp != 1;bp = next_free_block(bp)){
...
inline void check_freeblock(void* bp){
	...
	if(GET_BOUNDARY(bp) !=1 && next_free_block(prev_free_block(bp)) != bp ){
		fprintf(stderr, "checkheap: pred: pred and succ doesn't match.\n");
		fprintf(stderr, "pred's next=%p, bp's pred = %p\n",next_free_block(prev_free_block(bp)),(char*)GET(bp));
		fprintf(stderr, "pred's address=%p, bp's address=%p\n",prev_free_block(bp),bp);
	}
	if(GET_BOUNDARY(bp + WSIZE) !=1 && prev_free_block(next_free_block(bp)) != bp)
...
inline int check_freelist(int i){
	...
	for(bp = get_list_first(i); (unsigned long int)bp != 1; bp = next_free_block(bp)){
 ...
 */
inline void printblock(void *bp){
	printf("bp=%p,h_a = %d, f_a = %d, h_s = %d, f_s = %d, ",bp,
