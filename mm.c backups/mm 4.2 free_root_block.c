...
int mm_init(void) {
	...
	if((heap_listp = mem_sbrk(FREE_LIST_NUM*MIN_BLKSIZE + 2*DSIZE)) == (void *)-1){
	...
	for(int i=0; i<FREE_LIST_NUM; i++){
		...
		PUT(root-WSIZE, PACK(MIN_BLKSIZE, 1)); 
		PUT(root+DSIZE, PACK(MIN_BLKSIZE, 1)); 
		PUT(root, 1); 
		PUT(root+WSIZE, 1);
	}
	
	heap_listp += FREE_LIST_NUM*MIN_BLKSIZE;
...
inline void *find_fit(size_t asize){
	...
	for(int i = free_list_no; i < FREE_LIST_NUM; i++){
		char *first_block = get_list_first(i);
		if(first_block == 0)
			continue;
		for(bp = first_block; (unsigned long int)bp != 1;bp = NEXT_F_BLKP(bp)){
 ...
 */
inline void delete_free_block(void *bp ){
	char *prevp = PREV_F_BLKP(bp);
	char *nextp = NEXT_F_BLKP(bp);
	
	PUT(prevp + WSIZE, nextp);
	if(nextp != (char*)1)
		PUT(nextp, prevp);
	
	dbg_printf("delete_free_block: list no %d,root back to %ld \n",find_free_list_no(GET_SIZE(HDRP(bp))),GET(prevp + WSIZE) );
	dbg_printblock(bp);
}
 ...
 */
inline void *get_list_root(int i){
    return free_listp + i*MIN_BLKSIZE;
}
/*
 * get_list_first - Get the first node for list i
 */
inline void *get_list_first(int i){
    return GET_ADDR(free_listp + i*MIN_BLKSIZE + WSIZE);
}
...
void mm_checkheap(int verbose) {
	...
	if ((free_listp + FREE_LIST_NUM*MIN_BLKSIZE) != heap_listp){
...
inline void check_freeblock(void* bp){
	...
	if( ((in_heap(GET_ADDR(bp)) != 1) && (GET_BOUNDARY(bp) !=1))  ||
		((in_heap(GET_ADDR(bp + WSIZE)) != 1) && (GET_BOUNDARY(bp + WSIZE) !=1))   ){
...
inline int check_freelist(int i){
	...
	for(bp = get_list_first(i); (unsigned long int)bp != 1; bp = NEXT_F_BLKP(bp)){
