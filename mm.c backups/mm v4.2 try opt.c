...
int mm_init(void) {
	...
	dbg_printf("mm_init: free_listp start from: %p\n",free_listp);
	dbg_printf("mm_init: heap start from: %p\n",heap_listp);
 ...
 * 4		5			6			7			 64
 * 129-192;	193-256;	257-320;	321-384 
 *
 * 8		9			10			11			
 * 385-512;	513-640;	641-896;	897-1152;
 * 12		13			14			15
	...
	for(int i=0; i<FREE_LIST_NO; i++){
		...
		for(bp = (char*)flp; (unsigned long int)bp != 1; bp = NEXT_F_BLKP(bp)){
			...
			if(size < min_size || size > max_size){
				...
				fprintf(stderr, "size=%d,free_list_no=%d.\n",(int)size,find_free_list_no(size));
			}
			*/
		}
